"""Tools the custom agent composes.

This is the whole point of Lab 3: a custom agent can wire up a *wider* palette of
tools than Genie One's routing exposes — Genie spaces, a live web MCP, **and** a
Unity Catalog function (write-back), plus plain Python. Each tool has a live path
(real Databricks calls) and a dry-run path (canned Sunny Bay data) so the app runs
...
class GenieTool:
    ...
    def ask(self, question: str) -> str:
        ...
        # Return the text attachment(s) Genie produced.
        parts = []
...
def web_search(query: str) -> str:
    ...
    # Live: TODO wire the you.com MCP registered in the AI Gateway (Lab 1, Step 4b) via
    # databricks_langchain.DatabricksMCPServer, following the agent-langgraph template.
    # Kept a graceful no-op so a missing MCP grant degrades instead of failing the plan.
...
def create_service_order(machine_id: str, fault_code: str, part_id: str,
    ...
    part_id, technician_notes)`. THIS is the accountable action — the lab gates it
    behind a human approval in the UI before it is ever called."""
    if config.DRY_RUN:
...
def llm_route(message: str, history: list[dict]) -> dict[str, Any]:
    ...
    msgs = [("system", _ROUTER_SYSTEM)]
    msgs += [(h["role"], h["content"]) for h in history[-4:]]
...
def _warehouse_id(w) -> str:
    ...
    wid = __import__("os").environ.get("WAREHOUSE_ID")
    if wid:
        return wid
...
def parse_fleet_answer(_answer: str) -> list[dict[str, Any]]:
    """Turn the maintenance/sales tool answers into structured rows.

    In dry-run we read the canonical `_FLEET` directly (deterministic); live agents
    would parse the Genie result. Kept separate so the control-flow code in agent.py
    stays about *orchestration*, not string parsing.
    """
